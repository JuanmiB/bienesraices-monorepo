...
const CardPropiedad = ({ result }) => {
    ...
    return (
        <>
            <div
                className="h-min max-w-6xl min-w-[280px] border-2 rounded-lg border-grey cursor-pointer md:hover:border-2 md:hover:border-[var(--color-primary)] transition-all duration-200"
                onClick={() => handleViewPropertie(result.id)}
            >
                    ...
                    <div className="md:max-w-md w-full">
                        <div className="bg-white shadow-xl rounded-lg overflow-hidden">
                            <div
                                ...
                                style={{ backgroundImage: `url(${result.mainImageUrl})` }}
                            >
                            </div>
                            <div className="p-4">
                                <p className="uppercase tracking-wide text-sm font-bold text-gray-700">{result.title}</p>
                                <p className="text-3xl text-gray-900">{result.currency} {Number(result.price)?.toLocaleString() || '0'}</p>
                            ...
                            <div className="flex p-4 border-t border-gray-300 text-gray-700">
                                <div className="flex-1 inline-flex items-center">
                                    ...
                                    <p><span className="text-gray-900 font-bold">{result.bedrooms}</span> Hab.</p>
                                </div>
                                <div className="flex-1 inline-flex items-center">
                                    <svg className="h-6 w-6 text-gray-600 fill-current mr-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                    ...
                                    <p><span className="text-gray-900 font-bold">{result.bathrooms}</span> Baños</p>
                                </div>
                            </div>
                            <div className="px-4 pt-3 pb-4 border-t border-gray-300 bg-gray-100">
                                ...
                                <div className="flex items-center pt-2">
                                    ...
                                    <div>
                                        <p className="font-bold text-gray-900">{`${result?.owner?.firstName || ''} ${result?.owner?.lastName || ''}`.trim()}</p>
                                        <p className="text-sm text-gray-700">{result?.owner?.email}</p>
                                    </div>
                                </div>
                            </div>
